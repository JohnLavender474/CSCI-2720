...
/* CONSTRUCTOR */
/* ========================================================================================================== */
T3SolverImplementation::T3SolverImplementation(std::string root_game_board) try: game_tree(new NaryTree<std::string>),
                                                                                 size(0) {
...
void T3SolverImplementation::solve(bool print_loading_status) {
	this->game_tree
			->insert(this->root_game_board, this->count(root_game_board, BLANK));
		...
		for (int n = 0; n < next_game_board.size(); n++) {
			if (next_game_board.at(n) == BLANK) {
				...
				if (this->is_winner(next_game_board_cpy, character)) {
					...
					game_tree->insert(next_game_board_cpy, 0);
				} else {
					q.push(next_game_board_cpy);
					game_tree->insert(next_game_board_cpy, count(next_game_board_cpy, BLANK));
				}
				size++;
...
bool T3SolverImplementation::is_winner(std::string game_board, char character) {
	for (int i = 0; i < game_board.size(); i += 3) {
		if (game_board.at(i) == character && game_board.at(i + 1) == character && game_board.at(i + 2) == character) {
			return true;
		}
	}
	for (int i = 0; i < 3; i += 1) {
		if (game_board.at(i) == character && game_board.at(i + 3) == character && game_board.at(i + 6) == character) {
			return true;
		}
	}
	if (game_board.at(0) == character && game_board.at(4) == character && game_board.at(8) == character) {
		return true;
	}
	return game_board.at(2) == character && game_board.at(4) == character && game_board.at(6) == character;
}
