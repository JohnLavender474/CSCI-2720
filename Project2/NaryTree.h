...
void NaryTree<T>::insert(T data_field, size_t max_num_children)
{
	if (this->root == nullptr)
	{
		...
		std::cout << "inserted root: " << data_field << " " << max_num_children << std::endl;
	}
	else
	{
		...
		while (!q.empty())
		{
			...
			for (size_t n = 0; n < parent->max_num_children; n++)
			{
				...
				else
				{
					...
					std::cout << "inserted child: " << data_field << " " << max_num_children << std::endl;
					return;
				}
			}
		}
...
void NaryTree<T>::delete_tree(NaryNode<T> * node)
{
	...
	else
	{
		...
		std::cout << "deleted node with val: " << node->data_field << std::endl;
		delete node;
	}
}
