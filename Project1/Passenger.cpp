...
ostream &operator<<(ostream &os, const Passenger &passenger) {
	os << passenger.getLastName() << ", " << passenger.getFirstName() << " : Seat #" << passenger.getSeatNo()
	   << " Booking #" << passenger.getBookingNo() << endl;
	return os;
}

bool Passenger::operator<(const Passenger &other) {
	int comp = this->lastname
			.compare(other.getLastName());
	return (comp == 0) ? this->firstname
			                     .compare(other.getFirstName()) < 0 : comp < 0;
}
...
bool Passenger::operator==(const Passenger &other) {
	int lastnameComp = this->lastname
			.compare(other.getLastName());
	int firstnameComp = this->firstname
			.compare(other.getFirstName());
