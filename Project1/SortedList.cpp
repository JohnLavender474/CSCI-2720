...
template<class T> int SortedList<T>::getLength() {
	return this->length;
}

template<class T> T &SortedList<T>::get(int index) {
	return this->getNode(index)
			->data;
}

template<class T> void SortedList<T>::insert(T data) {
	if (this->length == 0) {
		this->head
				->data = data;
		this->length++;
...
template<class T> void SortedList<T>::remove(int index) {
...
template<class T> int SortedList<T>::indexOf(T data) {
...
template<class T> void SortedList<T>::display() {
	if (this->head == NULL) {
		cout << "NULL" << endl;
	} else {
		Node<T> *temp = this->head;
...
template<class T> void SortedList<T>::indexedInsert(int index, T data) {
	if (index < 0 || index > this->length) {
		throw invalid_argument("invalid index");
	} else if (index == 0) {
...
template<class T> Node<T> *SortedList<T>::getNode(int index) {
	if (index == 0) {
		return this->head;
