...
template<typename K, typename V> Entry<K, V>::Entry(K key, V value) :
		key(key), value(value) {
}

template<typename K, typename V> bool Entry<K, V>::operator==(Entry<K, V> const &other) const {
	return key == other.key && value == other.value;
}

template<typename K, typename V> void Entry<K, V>::print() {
	std::cout << "key: " << key << " value: " << value << std::endl;
}

template<typename K, typename V> void Entry<K, V>::set_value(V _value) {
	this->value = _value;
}
...
template<typename K, typename V> void Entry<K, V>::set_key(K _key) {
	this->key = _key;
}
...
template<typename K, typename V> HashMap<K, V>::HashMap() :
		prime_bit(1), num_entries(0), buckets(nullptr) {
	rehash();
}
...
template<typename K, typename V> void HashMap<K, V>::put(K key, V value) {
	...
	for (size_t i = 0; i < buckets[hash]->get_size(); i++) {
...
template<typename K, typename V> bool HashMap<K, V>::get(K key, V &val_ref) {
	size_t hash = hash_func(key);
...
template<typename K, typename V> bool HashMap<K, V>::contains(K key) {
	size_t hash = hash_func(key);
	LinkedList<Entry<K, V>> *bucket = buckets[hash];
	for (size_t i = 0; i < bucket->get_size(); i++) {
		Entry<K, V> entry = bucket->get(i);
...
template<typename K, typename V> size_t HashMap<K, V>::hash_func(K key) {
...
template<typename K, typename V> size_t HashMap<K, V>::get_size() {
	return num_entries;
}
...
template<typename K, typename V> size_t HashMap<K, V>::prime_size() {
	return 6 * ++prime_bit + 1;
}

template<typename K, typename V> float HashMap<K, V>::get_load_factor() {
